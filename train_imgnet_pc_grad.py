...
def train_student(args, student_model, student_normalize, clip_model, clip_normalize,
    ...
    # Setup training
    optimizer = PCGrad(torch.optim.AdamW(student_model.parameters(), lr=args.lr))
    ...
    for epoch in range(args.train_epoch):
        ...
        for unlabeled_imgs, _ in tqdm(train_loader_unlabeled, desc='Training',
            ...
            ce_loss = 0.5 * ce_loss
            distill_loss = 0.5 * (distill_loss_labeled + distill_loss_unlabeled)

            optimizer.zero_grad()
            optimizer.pc_backward([ce_loss, distill_loss])
            optimizer.step()
            scheduler.step()

            total_loss += (ce_loss + distill_loss).item()
        ...
        test_acc = evaluate(student_model, test_loader, student_normalize, search_param=False)
    ...
    final_acc = evaluate(student_model, test_loader, student_normalize, search_param=False)
